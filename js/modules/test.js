/* =========================================
   الوحدة: اختبار الحفظ (Test) - الإصلاح الشامل
...
window.TestHifz = () => {
    // حماية: التأكد من تحميل البيانات
    if (!window.APP_DATA || !window.APP_DATA.quran || Object.keys(window.APP_DATA.quran).length === 0) {
        return <div className="text-center p-6 text-gray-500 animate-pulse">⏳ جاري تجهيز المصحف للاختبار...</div>;
    }
    ...
    const generate = (isNext=false) => {
        ...
        if(isNext && currentQ) { 
            ...
            if(aIdx>=sObj.ayahs.length) return alertUser("انتهت", "انتهت السورة، اختر سؤالاً جديداً."); 
        }
        else {
            let pool=[];
            ...
            if(pool.length===0) return alertUser("تنبيه", "الرجاء تحديد سور للاختبار");
            
            // فلترة السور الموجودة فعلياً
            const valid = pool.filter(id => window.APP_DATA.quran[id]); 
            if(valid.length===0) return alertUser("خطأ", "لا توجد بيانات للسور المختارة");
        ...
        // محاولة جلب الصفحة (إذا توفرت البيانات)
        let pageAns = "-";
        if (qType==='page' && window.APP_DATA.pages) {
             ...
             if(pg) pageAns = pg.page;
        }

        setCurrentQ({sId, aIdx, qText, fullText:ayah.text, ansText: qType==='page' ? pageAns : ayah.text, info:`${sObj.name} - ${ayah.num}`, prompt});
        
        // تعديلات الإجابة حسب النوع
        if(qType==='next') setCurrentQ(p=>({...p, ansText:(aIdx+1<sObj.ayahs.length?sObj.ayahs[aIdx+1].text:"نهاية السورة")}));
        if(qType==='prev') setCurrentQ(p=>({...p, ansText:(aIdx>0?sObj.ayahs[aIdx-1].text:"بداية السورة")}));
        if(qType==='ayahNum') setCurrentQ(p=>({...p, ansText:ayah.num})); 
        if(qType==='surahName') setCurrentQ(p=>({...p, ansText:sObj.name}));
        
        setShowAns(false);
    };

    return (
        <div className="bg-white p-4 rounded-[2rem] border border-gray-200 shadow-sm animate-in">
