/* =========================================
   ...
   (شامل: نظام الإشعارات، المحاكي القرآني، واحة الزوار المحدثة)
   ========================================= */

const { useState, useEffect } = React;

// 1. استيراد المكونات (تم استخدام window لضمان العمل مع التجزئة)
...
const CustomModal = window.CustomModal; // النافذة المنبثقة
...
const App = () => {
    ...
    const [activeFeature, setActiveFeature] = useState(null); // للتحكم في أزرار ركن الطالب
    const [dataReady, setDataReady] = useState(false);
    
    // بيانات الطالب المحلية
    ...
    // 🔔 نظام استقبال الإشعارات (للموقع والهاتف)
    useEffect(() => {
        if (!window.db) return;
        ...
        const unsub = window.onSnapshot(window.doc(window.db, "appData", "notifications"), (doc) => {
            if (doc.exists()) {
                ...
                // إذا كان هناك إشعار جديد ونشط
                if (data.id && data.id !== lastMsgId && data.active) {
                    
                    // 1. إظهار النافذة داخل الموقع
                    if(window.showGlobalAlert) window.showGlobalAlert(data.title, data.body);
                    
                    // 2. إظهار إشعار الهاتف (System Notification)
                    if (Notification.permission === 'granted') {
                        new Notification(data.title, {
                            body: data.body,
                            icon: 'icon-192.png' // تأكد من وجود أيقونة بهذا الاسم
                        });
                    } else if (Notification.permission !== 'denied') {
                        // طلب الإذن إذا لم يرفض سابقاً
                        Notification.requestPermission();
                    }

                    // تخزين المعرف لمنع التكرار
            ...
            <main className="p-4 pb-24 animate-in">
                ...
                {page === 'student_corner' && (
                    <div className="space-y-4 max-w-lg mx-auto">
                        ...
                        <div onClick={() => toggleFeature('test')} className={`student-btn ${activeFeature === 'test' ? 'active' : ''}`}><span>🧠 اختبر حفظك (قديم)</span><span>{activeFeature === 'test'?'➖':'➕'}</span></div>{activeFeature === 'test' && (dataReady ? <TestHifz /> : <div className="text-center p-4">جاري التحميل...</div>)}
                ...
                {page === 'extras' && (
                    <div className="space-y-4 max-w-lg mx-auto">
                        ...
                        {/* المحاكي القرآني (أكورديون داخلي، لذا يظهر مباشرة) */}
                        {/* ملاحظة: المحاكي الجديد يحتوي على زر فتح خاص به، لذا نضعه مباشرة بدون زر خارجي ليكون مثل منبه الأوقات */}
                        {window.QuranExam && <window.QuranExam />}

                        {/* صانع البطاقات (زر قابل للفتح) */}
