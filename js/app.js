/* =========================================
   ...
   الوظيفة: تجميع الوحدات + إدارة النافذة العامة
   ========================================= */

const { useState, useEffect } = React;

// استيراد المكونات من الوحدات (Modules)
...
const App = () => {
    ...
    // --- نظام النافذة العامة (Global Modal System) ---
    const [modal, setModal] = useState({ show: false, title: '', msg: '' });

    // هذه الدالة السحرية تجعل أي ملف آخر قادر على استدعاء النافذة
    useEffect(() => {
        window.showGlobalAlert = (title, msg) => {
    ...
    return (
        <div id="app-container">
            {/* النافذة العامة التي ستظهر فوق كل شيء */}
            <CustomModal 
                isOpen={modal.show} 
            ...
            <main className="p-4 pb-24 animate-in">
                ...
                {page === 'card' && (<div className="max-w-md mx-auto space-y-6"><div className="bg-white p-8 rounded-[2.5rem] shadow-xl text-center border-4 border-emerald-50"><h2 className="text-2xl font-black mb-6">بيانات الطالب</h2><input value={studentName} onChange={e => {setStudentName(e.target.value); localStorage.setItem('st_name', e.target.value)}} className="w-full p-4 bg-gray-50 border rounded-2xl mb-3 text-center font-bold" placeholder="الاسم" /><button onClick={() => { setPage('home'); window.showGlobalAlert('نجاح', 'تم حفظ البيانات بنجاح ✅'); }} className="w-full bg-emerald-600 text-white py-4 rounded-2xl font-black shadow-lg">حفظ وتفعيل</button></div></div>)}
            </main>
