/* =========================================
   ملف التطبيق الرئيسي: js/app.js (مع نظام الإشعارات)
   ========================================= */
...
const App = () => {
    ...
    // ========================================================
    // 🔥 هذا هو الكود الجديد للإشعارات (تمت إضافته هنا)
    // ========================================================
    useEffect(() => {
        if (!window.db) return;
        ...
        const unsub = window.onSnapshot(window.doc(window.db, "appData", "notifications"), (doc) => {
            if (doc.exists()) {
                ...
                // إذا كان هناك إشعار جديد لم يره المستخدم من قبل
                if (data.id && data.id !== lastMsgId && data.active) {
                    ...
                    if (Notification.permission === 'granted') {
                        new Notification(data.title, {
                            body: data.body,
                            icon: 'icon-192.png' // تأكد من وجود هذه الصورة في ملفاتك
                        });
                    }

                    // تخزين المعرف حتى لا يظهر مرة أخرى لنفس الرسالة
