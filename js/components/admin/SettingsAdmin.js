/* =========================================
   المكون: إعدادات النصوص ومن نحن (المطور - دعم كل المنصات)
   المسار: js/components/admin/SettingsAdmin.js
   ========================================= */
const SettingsAdmin = ({ config, setConfig }) => {
    
    // دالة مساعدة لتحديث بيانات منصة معينة
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-emerald-50/50 p-4 rounded-xl border border-emerald-100 shadow-sm">
                ...
                {/* الأساسيات (واتساب وموقع) */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                    <div className="relative">
                        <div className="flex justify-between mb-1"><label className="text-[10px] font-bold text-green-700">واتساب:</label><input type="checkbox" checked={config.texts.contact.phone?.active !== false} onChange={e=>updatePlatform('phone', 'active', e.target.checked)} className="accent-green-600"/></div>
                        <input className={`w-full p-2 border border-green-200 rounded-lg text-left text-sm ${config.texts.contact.phone?.active===false ? 'opacity-50 bg-gray-100' : 'bg-white'}`} placeholder="9677xxxxxxxx" value={config.texts.contact.phone?.val || config.texts.contact.phone || ''} onChange={e=>updatePlatform('phone', 'val', e.target.value)} />
                    </div>
                    <div className="relative">
                ...
                <div className="bg-white p-3 rounded-xl border border-gray-200 mb-3 space-y-3">
                    ...
                    {platforms.map(p => (
                        <div key={p.id} className="flex items-center gap-2">
                            <input 
                                type="checkbox" 
                                checked={config.texts.contact[p.id]?.active === true} 
                                onChange={e => updatePlatform(p.id, 'active', e.target.checked)}
                                className="w-4 h-4 accent-emerald-600 cursor-pointer"
                            />
                            <span className={`w-20 text-[10px] font-bold ${p.color}`}>{p.icon} {p.label}:</span>
                            <input 
                                className={`flex-1 p-1.5 border rounded text-xs transition ${config.texts.contact[p.id]?.active ? 'bg-white border-gray-300' : 'bg-gray-100 border-gray-100 text-gray-400'}`} 
                                value={config.texts.contact[p.id]?.val || ''} 
                                onChange={e => updatePlatform(p.id, 'val', e.target.value)}
                                placeholder={`رابط ${p.label}`}
                                disabled={!config.texts.contact[p.id]?.active}
                            />
                        </div>
                    ))}
                </div>
