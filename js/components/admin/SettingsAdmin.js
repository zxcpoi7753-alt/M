/* =========================================
   المكون: إعدادات النصوص + مركز الإشعارات (النسخة الآمنة)
...
const SettingsAdmin = ({ config, setConfig }) => {
    ...
    // دالة إرسال الإشعار (محمية من الأخطاء)
    const sendNotification = async () => {
        if(!notify.title || !notify.body) return alert('الرجاء كتابة العنوان والرسالة');
        
        // استخدام window.confirm لتجنب مشاكل المتصفح
        if (!window.confirm('⚠️ هل أنت متأكد؟\nسيصل هذا الإشعار لجميع مستخدمي الموقع فوراً!')) return;

        try {
            // التحقق من وجود دالة setDoc
            if (!window.setDoc || !window.db) {
                throw new Error("خطأ في الاتصال بقاعدة البيانات (setDoc missing). تأكد من تحديث ملف firebase.js");
            }
        ...
        <div className="space-y-6">
            ...
            <div className="bg-gradient-to-br from-indigo-50 to-blue-50 p-4 rounded-xl border border-indigo-100 shadow-sm">
                ...
                <div className="space-y-3">
                    <input className="w-full p-3 border rounded-xl font-bold focus:ring-2 ring-indigo-200 outline-none" placeholder="عنوان الإشعار (مثلاً: 📢 تنبيه هام)" value={notify.title} onChange={e => setNotify({...notify, title: e.target.value})} />
                    <textarea className="w-full p-3 border rounded-xl h-20 resize-none text-sm focus:ring-2 ring-indigo-200 outline-none" placeholder="نص الرسالة..." value={notify.body} onChange={e => setNotify({...notify, body: e.target.value})} />
                    <button onClick={sendNotification} className="w-full bg-indigo-600 text-white py-3 rounded-xl font-black shadow-lg hover:bg-indigo-700 transition transform active:scale-95">🚀 إرسال للجميع الآن</button>
            ...
            <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
                ...
                <div className="flex items-center gap-4 bg-gray-50 p-2 rounded-lg">
                    ...
                    <input type="range" min="0.8" max="1.3" step="0.05" className="flex-1 h-2 bg-gray-300 rounded-lg appearance-none cursor-pointer accent-emerald-600" value={config.settings?.layoutScale || 1} onChange={e => setConfig({...config, settings: { ...config.settings, layoutScale: parseFloat(e.target.value) } })} />
            ...
            <div className="bg-emerald-50/50 p-4 rounded-xl border border-emerald-100 shadow-sm">
                ...
                <div className="bg-white p-3 rounded-xl border border-gray-200 mb-3 space-y-3">
                    <p className="text-[10px] font-bold text-gray-400 text-left border-b pb-1">منصات التواصل:</p>
                    {platforms.map(p => (
