...
const AdminApp = () => {
    // بيانات افتراضية فورية لتجنب شاشة التحميل البيضاء
    const [config, setConfig] = useState({
        settings: { layoutScale: 1 },
        texts: { siteTitle: 'جاري التحميل...', contact: {} },
        news: [], teachers: [], halaqat: [], schedules: []
    });
    
    // حالة اتصال فقط (لا تمنع عرض الصفحة)
    const [isSyncing, setIsSyncing] = useState(true);
    const [activeTab, setActiveTab] = useState(null);
    const [toast, setToast] = useState(null);

    // الاتصال بالبيانات (يعمل في الخلفية)
    useEffect(() => {
    ...
    // --- واجهة المستخدم تظهر فوراً ---
    return (
        <div id="app-container" className="p-4 pb-24 max-w-3xl mx-auto">
            {/* مؤشر حالة الاتصال الصغير في الأعلى */}
            ...
            {/* الأقسام تظهر فوراً حتى لو البيانات قديمة قليلاً */}
            <AdminSection id="txt" title="📝 النصوص الأساسية" activeTab={activeTab} setActiveTab={setActiveTab}>
                ...
                <textarea className="w-full p-3 border rounded-xl h-20" placeholder="رسالة الطالب" value={config.texts.studentMsg} onChange={e=>setConfig({...config, texts:{...config.texts, studentMsg:e.target.value}})} />
            </AdminSection>
            ...
            <AdminSection id="news" title="📰 الأخبار" activeTab={activeTab} setActiveTab={setActiveTab}>
                ...
                {config.news.map(n => (
                    <div key={n.id} className="border p-3 rounded-xl mb-2">
                        ...
                        <button onClick={()=>{setConfig({...config, news:config.news.filter(x=>x.id!==n.id)})}} className="text-red-500 text-xs font-bold mt-1">حذف 🗑️</button>
                    </div>
                ))}
            </AdminSection>

            {/* باقي الأقسام (المعلمون، الجداول..) بنفس الطريقة */}
            <p className="text-center text-gray-400 text-xs mt-8">نسخة لوحة التحكم السريعة V2</p>
        </div>
    );
};
