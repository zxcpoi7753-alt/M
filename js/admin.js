...
const AdminSection = ({ id, title, activeTab, setActiveTab, children }) => (
    <div className="admin-card animate-in bg-white border border-gray-100 mb-4 rounded-xl shadow-sm overflow-hidden">
        <div 
            onClick={() => setActiveTab(activeTab === id ? null : id)} 
            className={`admin-section-btn p-4 flex justify-between items-center cursor-pointer transition-colors ${activeTab === id ? 'bg-emerald-50 text-emerald-800' : 'hover:bg-gray-50'}`}
        >
...
const AdminApp = () => {
    ...
    useEffect(() => {
        const unsub = onSnapshot(doc(db, "appData", "mainConfig"), (docSnapshot) => {
            if (docSnapshot.exists()) {
                ...
                // دمج البيانات لضمان وجود حقول التواصل الجديدة
    ...
    // شاشة التحميل (لحل مشكلة الشاشة البيضاء المؤقتة)
    if (loading) return (
        <div className="flex justify-center items-center h-screen flex-col bg-gray-50">
            <div className="w-12 h-12 border-4 border-emerald-500 border-t-transparent rounded-full animate-spin mb-4"></div>
            <p className="font-bold text-gray-500 text-sm">جاري الاتصال بقاعدة البيانات...</p>
        </div>
    );

    return (
        <div id="app-container" className="p-4 pb-24 max-w-3xl mx-auto">
            <h1 className="text-3xl font-black text-emerald-800 mb-6 text-center mt-4">⚙️ مركز القيادة</h1>
            
            {/* زر الحفظ العائم */}
            <button onClick={handleSave} className="fab-save hover:scale-105 transition">💾 حفظ التعديلات</button>
            ...
            <AdminSection id="settings" title="1. أحجام العرض 📏" activeTab={activeTab} setActiveTab={setActiveTab}>
                <div><label className="text-xs font-bold text-gray-500 block mb-2">حجم واجهة الموقع (Zoom)</label><input type="range" min="0.5" max="1.2" step="0.05" className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-emerald-600" value={config.settings.layoutScale} onChange={e => setConfig({ ...config, settings: { ...config.settings, layoutScale: parseFloat(e.target.value) } })} /></div>
            </AdminSection>
            ...
            <AdminSection id="texts" title="2. النصوص ورسائل الطالب 📝" activeTab={activeTab} setActiveTab={setActiveTab}>
                <div className="space-y-3">
                    ...
                    <input className="w-full p-3 border rounded-xl font-bold text-sm" placeholder="عنوان الترحيب" value={config.texts.heroTitle} onChange={e => setConfig({...config, texts: {...config.texts, heroTitle: e.target.value}})} />
                    <div className="bg-amber-50 p-3 rounded-xl border border-amber-200"><label className="text-xs font-black text-amber-700 block mb-1">رسالة ركن الطالب:</label><textarea className="w-full p-3 border rounded-xl h-20 text-sm" value={config.texts.studentMsg} onChange={e => setConfig({...config, texts: {...config.texts, studentMsg: e.target.value}})} /></div>
                    <div className="bg-emerald-50 p-3 rounded-xl border border-emerald-200"><label className="text-xs font-black text-emerald-700 block mb-1">سؤال الأسبوع:</label><textarea className="w-full p-3 border rounded-xl h-20 text-sm" value={config.texts.weeklyQuestion} onChange={e => setConfig({...config, texts: {...config.texts, weeklyQuestion: e.target.value}})} /></div>
                </div>
            </AdminSection>

            {/* 3. الأخبار */}
            <AdminSection id="news" title="3. الأخبار والإعلانات 📰" activeTab={activeTab} setActiveTab={setActiveTab}>
                <button onClick={() => setConfig({ ...config, news: [{ id: Date.now(), title: '', date: new Date().toISOString().split('T')[0], content: '', hidden: false, colors: { title: '#000', content: '#555', link: '#2563eb' }, link: { url: '', text: '' } }, ...config.news] })} className="w-full bg-emerald-100 text-emerald-700 py-3 rounded-xl font-black mb-4 hover:bg-emerald-200 text-sm">+ إضافة خبر جديد</button>
                {config.news.map((item) => (
                    <div key={item.id} className={`p-4 border rounded-2xl mb-3 relative bg-white ${item.hidden ? 'item-hidden' : ''}`}>
                        <div className="flex justify-end gap-2 mb-2 border-b pb-2"><button onClick={() => setSecurityModal({ show: true, action: 'hide', type: 'news', id: item.id })} className="text-[10px] bg-amber-100 text-amber-800 px-3 py-1 rounded-lg font-bold">{item.hidden ? 'إظهار' : 'إخفاء'}</button><button onClick={() => setSecurityModal({ show: true, action: 'delete', type: 'news', id: item.id })} className="text-[10px] bg-red-100 text-red-800 px-3 py-1 rounded-lg font-bold">حذف</button></div>
                        <input className="w-full p-2 font-black border rounded-lg mb-2 text-sm" placeholder="العنوان" value={item.title} onChange={e => { const list = config.news.map(x => x.id === item.id ? { ...x, title: e.target.value } : x); setConfig({ ...config, news: list }); }} />
            ...
            <AdminSection id="teachers" title="4. المعلمون 👨‍🏫" activeTab={activeTab} setActiveTab={setActiveTab}>
                <button onClick={() => setConfig({ ...config, teachers: [...config.teachers, { id: Date.now(), name: 'معلم جديد', bio: '', avatar: '🧔', hidden: false }] })} className="w-full bg-blue-50 text-blue-600 py-3 rounded-xl font-black mb-4 text-sm">+ إضافة معلم</button>
            ...
            <AdminSection id="schedules" title="5. الجداول الدراسية 📅" activeTab={activeTab} setActiveTab={setActiveTab}>
                ...
                {config.schedules.map((sch) => (
                    <div key={sch.id} className={`p-3 border rounded-xl mb-3 bg-white ${sch.hidden ? 'item-hidden' : ''}`}>
                        <div className="flex justify-between items-center mb-2 border-b pb-2"><span className="font-bold text-emerald-800 text-sm">{sch.name} ({sch.period})</span><div className="flex gap-1"><button onClick={() => setSecurityModal({ show: true, action: 'hide', type: 'schedule', id: sch.id })} className="text-[10px] bg-amber-100 px-2 py-1 rounded">👁️</button><button onClick={() => setSecurityModal({ show: true, action: 'delete', type: 'schedule', id: sch.id })} className="text-[10px] bg-red-100 px-2 py-1 rounded">🗑️</button></div></div>
            ...
            <AdminSection id="top" title="6. أوائل الحلقات 🏆" activeTab={activeTab} setActiveTab={setActiveTab}>
                <button onClick={() => { const name = prompt('اسم الحلقة:'); if (name) setConfig({ ...config, halaqat: [...config.halaqat, { id: Date.now(), name, students: [], hidden: false }] }); }} className="w-full bg-amber-100 text-amber-800 py-3 rounded-xl font-black mb-4 text-sm">+ إضافة حلقة</button>
                {config.halaqat.map((h) => (
                    <div key={h.id} className={`p-4 bg-amber-50 rounded-2xl mb-4 border border-amber-200 ${h.hidden ? 'item-hidden' : ''}`}>
                        <div className="flex justify-between items-center mb-2 pb-2 border-b border-amber-200"><span className="font-bold text-amber-900 text-sm">{h.name}</span><div className="flex gap-2"><button onClick={() => setSecurityModal({ show: true, action: 'hide', type: 'halqa', id: h.id })} className="text-[10px] text-amber-600 font-bold">👁️</button><button onClick={() => setSecurityModal({ show: true, action: 'delete', type: 'halqa', id: h.id })} className="text-[10px] text-red-600 font-bold">🗑️</button></div></div>
                        {h.students.map((st) => (
            ...
            <AdminSection id="about" title="7. بيانات 'من نحن' 🔗" activeTab={activeTab} setActiveTab={setActiveTab}>
                <div className="space-y-3">
                    <textarea className="w-full p-3 border rounded-xl h-24 text-sm" placeholder="المحتوى التعريفي" value={config.texts.aboutMain} onChange={e => setConfig({...config, texts: {...config.texts, aboutMain: e.target.value}})} />
                    <input className="w-full p-3 border rounded-xl text-sm" placeholder="الآية القرآنية" value={config.texts.aboutAyah} onChange={e => setConfig({...config, texts: {...config.texts, aboutAyah: e.target.value}})} />
                    <input className="w-full p-3 border rounded-xl text-sm" placeholder="الخاتمة" value={config.texts.aboutFooter} onChange={e => setConfig({...config, texts: {...config.texts, aboutFooter: e.target.value}})} />
                    
                    <div className="bg-blue-50 p-4 rounded-xl border border-blue-200 space-y-3">
                        <label className="text-xs font-bold text-blue-800 block mb-1">🔗 روابط ومنصات التواصل:</label>
                        <div className="grid grid-cols-1 gap-2">
                            <input className="p-2 border rounded text-xs" placeholder="رقم واتساب (مثلاً 96777000000)" value={config.texts.contact.phone} onChange={e => setConfig({...config, texts: {...config.texts, contact: {...config.texts.contact, phone: e.target.value}}})} />
                            <input className="p-2 border rounded text-xs" placeholder="رابط الموقع (Google Maps)" value={config.texts.contact.location} onChange={e => setConfig({...config, texts: {...config.texts, contact: {...config.texts.contact, location: e.target.value}}})} />
                            <input className="p-2 border rounded text-xs" placeholder="رابط قناة اليوتيوب" value={config.texts.contact.youtube} onChange={e => setConfig({...config, texts: {...config.texts, contact: {...config.texts.contact, youtube: e.target.value}}})} />
                            <input className="p-2 border rounded text-xs" placeholder="رابط صفحة الفيسبوك" value={config.texts.contact.facebook} onChange={e => setConfig({...config, texts: {...config.texts, contact: {...config.texts.contact, facebook: e.target.value}}})} />
                            <input className="p-2 border rounded text-xs" placeholder="رابط الانستقرام" value={config.texts.contact.instagram} onChange={e => setConfig({...config, texts: {...config.texts, contact: {...config.texts.contact, instagram: e.target.value}}})} />
                        </div>
                    </div>
                </div>
            </AdminSection>

            <button onClick={() => { sessionStorage.removeItem('thuraya_admin_auth'); window.location.href = "index.html"; }} className="w-full py-4 mt-8 bg-red-600 text-white rounded-2xl font-black shadow-xl hover:bg-red-700 transition">تسجيل خروج 🔒</button>
        </div>
    );
};
